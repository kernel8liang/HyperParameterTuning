"""Do a line search along the line given by the initial gradient."""
import pickle
...
from hypergrad.nn_utils import make_nn_funs, VectorParser, logit, fill_parser
...
init_log_alphas = 0.884   # Based on 200 iterations of whetlab.
init_invlogit_betas = 0.58
init_log_param_scale = -1.94

# ----- Superparameters -----
N_gradients_in_average = 1
N_points_in_line_search = 50
stepsize_search_rescale = 50
...
def plot():
    ...
    with open('results.pkl') as f:
        results, parser, first_gradient, parsed_avg_grad, stepsizes = pickle.load(f)

    best_ix = np.argmin(results['train_loss'])

    # ----- Alpha and beta initial hypergradients -----
    fig = plt.figure(0)
    fig.clf()
    ax = fig.add_subplot(411)
    for cur_results, name in zip(parsed_avg_grad['log_alphas'].T, parser.names):
        if name[0] == 'weights':
    ...
    for cur_results, name in zip(parsed_avg_grad['invlogit_betas'].T, parser.names):
        if name[0] == 'weights':
            ax.plot(cur_results, 'o-', label=name)
    ...
    for cur_results, name in zip(results['log_alphas'][best_ix].T, parser.names):
        if name[0] == 'weights':
    ...
    ax = fig.add_subplot(412)
    #ax.set_title('Alpha learning curves')
    for cur_results, name in zip(results['invlogit_betas'][best_ix].T, parser.names):
        if name[0] == 'weights':
