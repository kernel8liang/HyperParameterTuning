...
def plot():
    ...
    for i in range(0, len(all_learning_curves), 10000) + [len(all_learning_curves) - 1]:
        log_alphas = all_learning_curves[i]
    ...
    for i, layer in enumerate(all_bindicts[0]):
        ...
        #for ix in range(0, len(all_bindicts), 10000) + [len(all_bindicts) - 1]:
        for ix in [len(all_bindicts) - 1]:
            bindict = all_bindicts[ix]
            bins = bindict[layer]
